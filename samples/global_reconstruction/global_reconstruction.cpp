...
typedef boost::gil::gray16_image_t image_ndvi_t;
...
void ndvi(const image_rgb_const_view_t& rgb, const image_pir_const_view_t& pir, const image_z_const_view_t& z, const image_zndvi_view_t& dst) {
    for (int y=0; y<dst.height(); ++y)
        for (int x=0; x<dst.width(); ++x) {
	unsigned short r = boost::gil::at_c<0>(rgb(x,y));
	unsigned short i = pir(x,y);
...
int main(int argc, char **argv) {
	...
	image_zndvi_t img_out(img_rgb.dimensions());
