	...
	wxDialog(parent, id, title, pos, size, style)
{
	...
	for (unsigned int i=0; i<pileText2.size(); ++i)
	{
		...
		m_pileText.push_back(make_pair(new wxStaticText(this, -1, wxString(pileText2[i].first.c_str(),*wxConvCurrent)),new wxTextCtrl(this, -1, wxString(pileText2[i].second.c_str(),*wxConvCurrent))));
		sizerH->Add(m_pileText.back().first, 0, wxALL|wxALIGN_CENTER_VERTICAL, 5);
		sizerH->Add(m_pileText.back().second, 1, wxALL, 5);
		test_sizer->Add(sizerH,0,wxEXPAND);
		sizerH->Layout();
	}
